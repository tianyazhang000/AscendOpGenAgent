---
name: op-task-prepare
description: >
  从用户 PyTorch/Python 代码中提取算子实现，构建为 NPUKernelBench 格式的标准化
  单文件自包含任务（model.py）。支持从独立的文件中提取 shape/dtype 信息。
argument-hint: >
  需要提供：1) 待优化的代码文件路径；
...
算子部分，并将其构建为 NPUKernelBench 格式的 model.py 文件。
</role>
...
详细格式规范见 `@references/npuKernelbnch-format.md`
...
```bash
python3 <skill-path>/scripts/validate_task.py /abs/path/model.py --json
```
...
验证通过后，**必须使用 `question` 工具**将完整的 model.py 内容展示给用户，请求确认。
...
- 在 `question` 的描述中展示 model.py 的完整内容
- 提供"确认"和"需要修改"两个选项
